...
function setUserResponse(message) {
    const user_response = `<img class="userAvatar" src='./static/images/userAvatar.png'><p class="userMsg">${message} </p><div class="clearfix"></div>`;
...
function getBotResponse(text) {
    botResponse = `<img class="botAvatar" src="./static/images/botAvatar_old.png"/><span class="botMsg">${text}</span><div class="clearfix"></div>`;
    return botResponse;
}
...
function setBotResponse(response) {
    ...
    setTimeout(() => {
        ...
        if (response.length < 1) {
            ...
            const fallbackMsg = "Estoy enfrentando algunos problemas, intente nuevamente más tarde.!!!";

            const BotResponse = `<img class="botAvatar" src="./static/images/botAvatar_old.png"/><p class="botMsg">${fallbackMsg}</p><div class="clearfix"></div>`;
                ...
                if (Object.hasOwnProperty.call(response[i], "text")) {
                    if (response[i].text != null) {
                        ...
                        if (html.includes("<ul") || html.includes("<ol") || html.includes("<li") || html.includes('<h3')) {
                            html = html.replaceAll("<br>", "");
                            // botResponse = `<img class="botAvatar" src="./static/images/botAvatar_old.png"/><span class="botMsg">${html}</span><div class="clearfix"></div>`;
                            botResponse = getBotResponse(html);
                        }
                        else {
                            // if no markdown formatting found, render the text as it is.
                            if (!botResponse) {
                                botResponse = `<img class="botAvatar" src="./static/images/botAvatar_old.png"/><p class="botMsg">${response[i].text}</p><div class="clearfix"></div>`;
                            }
                        }
